					   ...
					   uint32_t max_queued_pkts)
{
	...
	if (rc < 0)
		return ODP_INT_QUEUE_POOL_INVALID;
	...
	pool->min_free_list_size = pool->free_list_size;
	pool->peak_free_list_size = pool->free_list_size;
	return (odp_int_queue_pool_t)pool;
}
...
odp_int_pkt_queue_t odp_pkt_queue_create(odp_int_queue_pool_t queue_pool)
{
	...
	if (pool->max_queue_num < queue_num)
		return ODP_INT_PKT_QUEUE_INVALID;

	return (odp_int_pkt_queue_t)queue_num;
}

int odp_pkt_queue_append(odp_int_queue_pool_t queue_pool,
			 odp_int_pkt_queue_t pkt_queue, odp_packet_t pkt)
{
	queue_pool_t *pool;
...
void odp_pkt_queue_stats_print(odp_int_queue_pool_t queue_pool)
{
	...
	ODP_DBG("  max_queue_num=%u max_queued_pkts=%u next_queue_num=%u\n",
		...
		pool->next_queue_num);
	ODP_DBG("  total pkt appends=%lu total pkt removes=%lu bad removes=%lu\n",
