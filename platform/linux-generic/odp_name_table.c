		...
		if (hash_tbl_entry != 0) {
			if ((hash_tbl_entry & 0x3F) != 0) {
				...
				if (entry_cnt != list_cnt)
					ODP_DBG("%s idx=%u entry_cnt=%u list_cnt=%u\n",
						__func__,
...
void odp_int_name_tbl_stats_print(void)
{
	...
	for (idx = 0; idx < NUM_NAME_TBLS; idx++) {
		...
		if ((name_tbl) && (name_tbl->num_used != 0))
			ODP_DBG("  name_tbl %u  num_allocd=%7u num_added_to_free_list=%7u num_used=%7u num_avail_to_add=%7u\n",
				idx,
				name_tbl->num_allocd,
	...
	ODP_DBG("  num of first level secondary hash tbls=%u second level tbls=%u\n",
