...
static profile_params_set_t COMPANY_PROFILE_PARAMS =
{
    ...
    .wred_params =
    {
        ...
        {
            .min_threshold = PERCENT(70),  .med_threshold     = PERCENT(90),
            .med_drop_prob = PERCENT(80),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },

        [PKT_RED] =
        {
            .min_threshold = PERCENT(40),  .med_threshold     = PERCENT(70),
            .med_drop_prob = PERCENT(70),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },
    }
};
...
static profile_params_set_t COS0_PROFILE_PARAMS =
{
    ...
    .wred_params =
    {
        [PKT_GREEN ... PKT_YELLOW] =
        {
            .min_threshold = PERCENT(80),  .med_threshold     = PERCENT(90),
            .med_drop_prob = PERCENT(50),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },

        [PKT_RED] =
        {
            .min_threshold = PERCENT(60),  .med_threshold     = PERCENT(80),
            .med_drop_prob = PERCENT(70),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },
    }
...
static profile_params_set_t COS1_PROFILE_PARAMS =
{
    ...
    .wred_params =
    {
        [PKT_GREEN ... PKT_YELLOW] =
        {
            .min_threshold = PERCENT(40),  .med_threshold     = PERCENT(90),
            .med_drop_prob = PERCENT(70),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },
        ...
        [PKT_RED] =
        {
            .min_threshold = PERCENT(50),  .med_threshold     = PERCENT(80),
            .med_drop_prob = PERCENT(80),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },
    }
...
static profile_params_set_t COS3_PROFILE_PARAMS =
{
    ...
    .wred_params =
    {
        ...
        [PKT_RED] =
        {
            .min_threshold = PERCENT(30),  .med_threshold     = PERCENT(60),
            .med_drop_prob = PERCENT(80),  .max_drop_prob     = PERCENT(100),
            .enable_wred   = TRUE,         .use_byte_fullness = FALSE,
        },
    }
...
static uint32_t next_rand_byte = 0;
...
extern void odp_tm_stats_print(odp_tm_t odp_tm);

extern void odp_int_name_tbl_init(void);
                                   ...
                                   uint32_t              threshold_scale)
{
    ...
    profile_set->threshold_profile    = odp_tm_threshold_create(name,
                                                                &threshold_params);
    ...
    for (color = 0; color < ODP_NUM_PKT_COLORS; color++)
    {
        ...
        profile_set->wred_profiles[color] = odp_tm_wred_create(wred_name,
                                                               &wred_params);
...
static uint32_t init_profile_sets(void)
{
    ...
    err_cnt += create_profile_set(&COMPANY_PROFILE_PARAMS, &COMPANY_PROFILE_SET,
                                  "CompanyProfiles", 0, 1, 1);

    err_cnt += create_profile_set(&COS0_PROFILE_PARAMS, &COS_PROFILE_SETS[0],
                                  "ServiceClass0", 0,
                                  class_shaper_scale, class_threshold_scale);
    err_cnt += create_profile_set(&COS1_PROFILE_PARAMS, &COS_PROFILE_SETS[1],
                                  "ServiceClass1", 0,
                                  class_shaper_scale, class_threshold_scale);
    err_cnt += create_profile_set(&COS2_PROFILE_PARAMS, &COS_PROFILE_SETS[2],
                                  "ServiceClass2", 0,
                                  class_shaper_scale, class_threshold_scale);
    err_cnt += create_profile_set(&COS3_PROFILE_PARAMS, &COS_PROFILE_SETS[3],
                                  "ServiceClass3", 0,
                                  class_shaper_scale, class_threshold_scale);

    err_cnt += create_profile_set(&COS0_PROFILE_PARAMS, &USER_PROFILE_SETS[0],
                                  "UserSvc0", 0,
                                  user_shaper_scale, user_threshold_scale);
    err_cnt += create_profile_set(&COS1_PROFILE_PARAMS, &USER_PROFILE_SETS[1],
                                  "UserSvc1", 0,
                                  user_shaper_scale, user_threshold_scale);
    err_cnt += create_profile_set(&COS2_PROFILE_PARAMS, &USER_PROFILE_SETS[2],
                                  "UserSvc2", 0,
                                  user_shaper_scale, user_threshold_scale);
    err_cnt += create_profile_set(&COS3_PROFILE_PARAMS, &USER_PROFILE_SETS[3],
                                  "UserSvc3", 0,
                                  user_shaper_scale, user_threshold_scale);
                               ...
                               uint32_t      user_num)
{
    ...
    tm_node_params.wred_profile[PKT_GREEN]  = profile_set->wred_profiles[0];
    tm_node_params.wred_profile[PKT_YELLOW] = profile_set->wred_profiles[1];
    tm_node_params.wred_profile[PKT_RED]    = profile_set->wred_profiles[2];
    tm_node_params.level                    = 2;

    snprintf(user_name, sizeof(user_name), "Subscriber-%u", user_num);
    user_tm_node = odp_tm_node_create(odp_tm_test, user_name, &tm_node_params);
    odp_tm_node_connect(user_tm_node, cos_tm_node);

    for (app_idx = 0; app_idx < APPS_PER_USER; app_idx++)
    {
        profile_set = &APP_PROFILE_SETS[svc_class][app_idx];
        for (queue_idx = 0; queue_idx < TM_QUEUES_PER_APP; queue_idx++)
        {
            odp_tm_queue_params_init(&tm_queue_params);
            tm_queue_params.shaper_profile    = profile_set->shaper_profile;
            tm_queue_params.threshold_profile = profile_set->threshold_profile;
            tm_queue_params.priority          = svc_class;

            tm_queue_params.wred_profile[PKT_GREEN]  =
                profile_set->wred_profiles[PKT_GREEN];
            tm_queue_params.wred_profile[PKT_YELLOW] =
                profile_set->wred_profiles[PKT_YELLOW];
            tm_queue_params.wred_profile[PKT_RED]    =
                profile_set->wred_profiles[PKT_RED];

            tm_queue = odp_tm_queue_create(odp_tm_test, &tm_queue_params);
            rc = odp_tm_queue_connect(tm_queue, user_tm_node);
            if (rc < 0)
                return rc;

            svc_class_queue_num = next_queue_nums[svc_class]++;
            queue_num_tbls[svc_class][svc_class_queue_num + 1] = tm_queue;
        }
    }
...
static int config_company_node(const char *company_name)
{
    ...
    for (cos_idx = 0; cos_idx < NUM_SVC_CLASSES; cos_idx++)
    {
        ...
        tm_node_params.wred_profile[PKT_GREEN]  =
            profile_set->wred_profiles[PKT_GREEN];
        tm_node_params.wred_profile[PKT_YELLOW] =
            profile_set->wred_profiles[PKT_YELLOW];
        tm_node_params.wred_profile[PKT_RED]    =
            profile_set->wred_profiles[PKT_RED];

        cos_tm_node = odp_tm_node_create(odp_tm_test, NULL, &tm_node_params);
        odp_tm_node_connect(cos_tm_node, company_tm_node);

        for (user_idx = 0; user_idx < USERS_PER_SVC_CLASS; user_idx++)
            config_example_user(cos_tm_node, cos_idx, cos_idx * 256 + user_idx);
    }

    odp_tm_node_connect(company_tm_node, ODP_TM_INVALID);
    return 0;
}
...
static int create_and_config_tm(void)
{
    ...
    params.egress.egress_kind                    = TM_EGRESS_FN;
...
static int traffic_generator(uint32_t pkts_to_send)
{
    ...
    odp_tm_queue_t   tm_queue;
    odp_packet_t     pkt;
    uint32_t         svc_class, queue_num, pkt_len, pkts_into_tm, pkts_from_tm;
    uint32_t         pkt_cnt, secs, odp_tm_enq_errs;
    int              rc;

    memset(&pool_params, 0, sizeof(odp_pool_param_t));
    pool_params.type           = ODP_POOL_PACKET;
    pool_params.pkt.num        = pkts_to_send;
    ...
    for (pkt_cnt = 1; pkt_cnt <= pkts_to_send; pkt_cnt++)
    {
    ...
    printf("%s odp_tm_enq_errs=%u\n", __FUNCTION__, odp_tm_enq_errs);

    // Wait for up to 8 seconds for pkts_from_tm to match pkts_into_tm.
        ...
        else if (arg[0] == '-')
        {
            ...
            case 'n':
                if (argc <= arg_idx)
                    ...
                    g_num_pkts_to_send = atoi(argv[arg_idx++]);
                break;
            ...
            return -1; // Currently all cmd line options are '-' flag based.
    }
